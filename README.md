...
- ✅ **KTO 句子长度更合理**：19.41 vs 7.28（KTO 更符合真实直播）
...
```
DPO 数据中的规律：
Chosen（短句）: "你们用过其他蓝牙耳机吗？地铁听不清对不对？"
Rejected（长句）: "这款蓝牙耳机含有通话清晰成分，能够有效改善地铁听不清问题。"

DPO 学到的模式：短句 = 互动性强 = 好
结果：DPO 模型把"短句"泛化到了所有输出
```

**KTO 的优势**：
- **不学习长度偏好**：只学"好坏"，不学"长短"
- **保留多样性**：可以有短句（互动），也可以有长句（讲卖点）
- **更符合真实直播**：主播会根据内容调整句子长度

**推荐**：**KTO 更适合有实时互动的数字人直播**，因为互动密度高、口语化好、流畅度接近满分、最省显存、句子长度更合理。
